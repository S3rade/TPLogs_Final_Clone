...
{ write_log(ADMIN_LOGIN_LOG,'Admin: '.$_SESSION["Username"].' ID: '.$_SESSION["id_for_admin"].' accessed '.$_SERVER['PHP_SELF'].' ');
  $token_age = time() - $_SESSION['token_time'];
    if ($token_age <= 3600) 
    {
        if ($_SESSION["Roles"]== "Admin")
        {
            ...
            {   
                    ...
                    !empty($_POST['DOB']))    
                {
                    ...
                    if ($query->execute()){  
                      write_log(ADMIN_UPDATE_LOG,'Updated Staff'.$Username.'By Admin ID of'.$_SESSION["Username"].' ID: '.$_SESSION["id_for_admin"].'  ');
                        echo "<script>alert('Successfully updated Staff'); window.location.href='add_staff.php'; </script>";
                    }else{ 
                      write_log(ADMIN_UPDATE_LOG_ERR,'FAILED Updated Staff '.$Username.'By Admin Username of'.$_SESSION["Username"].' ID: '.$_SESSION["id_for_admin"].'   ');
        ...
        else{write_log(ADMIN_LOGIN_LOG_ERR,$_SESSION["Username"].' ID: '.$_SESSION["id_for_admin"].' tried accessing '.$_SERVER['PHP_SELF'].' ');session_destroy();header ('location: login_Admin_Staff.php');}
    }
    else{write_log(ADMIN_LOGIN_LOG_ERR,$_SESSION["Username"].' ID: '.$_SESSION["id_for_admin"].' tried accessing '.$_SERVER['PHP_SELF'].' ');session_destroy();header ('location: login_Admin_Staff.php');}
}
...
<a href="add_staff.php"><input type="button" value="BACK" /></a>
</html>
